...
parser = argparse.ArgumentParser(description='convert atmos model output to overturning streamfunction')
parser.add_argument('infile', help='input file on model levels containing vcomp, temp, pfull, lat')
parser.add_argument('outfile', help='output file containing streamfunction variables')
args = parser.parse_args()

psi, psis = climate.ComputePsi(data=args.infile)
...
pfull_a = f.variables['pfull'].__dict__
lat = f.variables['lat'][:]
...
p_o = f.createVariable('pfull','f8', ('pfull'))
p_o.setncatts(pfull_a)
p_o[:] = pfull[:]
...
psi_o = f.createVariable('psi', 'f4', ('time', 'pfull', 'lat'), fill_value=-1.e10)
psi_o.units = '10^9 kg/s'
psi_o.long_name = 'Meridional Overturning Streamfunction'
psi_o[:] = psi[:]

psis_o = f.createVariable('psis', 'f4', ('time', 'pfull', 'lat'), fill_value=-1.e10)
