...
ty = np.ma.sum(ty, axis=3)
ty_rho = np.ma.sum(ty_rho, axis=3)
ty_gm = np.ma.sum(ty_gm, axis=3)
ty_rho_gm = np.ma.sum(ty_rho_gm, axis=3)
...
moc = moc * 1.0e-9
moc_tot = moc_tot * 1.0e-9
moc_rho = moc_rho * 1.0e-9
moc_rho_tot = moc_rho_tot * 1.0e-9
ty_gm = ty_gm * 1.0e-9
ty_rho_gm = ty_rho_gm * 1.0e-9
